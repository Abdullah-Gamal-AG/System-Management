...
public partial class ProudectPerformance : Form
{
	private TextBox proudectname;
	...
	private TextBox numberofquntitysold;

	private TextBox numberoforders;


	public ProudectPerformance()
	{
		...
		{
			Text = "Number of Quantity Sold",
			Font = new Font("Segoe UI", 9f, FontStyle.Bold),
			ForeColor = Color.DimGray,
			Location = new Point(15, 176),
			AutoSize = true
		};
		numberofquntitysold = new TextBox
		{
			Location = new Point(15, 199),
			Width = 248,
			ReadOnly = true,
			BackColor = Color.White,
			PlaceholderText = "Sold quantity will appear here"
		};
		...
		panel2.Controls.AddRange(label3, label4, totalProfit, label5, numberoforders, label6, numberofquntitysold, label7);
		num2 += 260;
		statue = new Label
		{
			Text = string.Empty,
		...
		base.Controls.AddRange(label, label2, panel, proudectname, startDate, endDate, panel2, statue);
		base.Load += ProudectPerformance_Load;
		proudectname.TextChanged += ProudectPerformance_Load;
		startDate.TextChanged += ProudectPerformance_Load;
		endDate.TextChanged += ProudectPerformance_Load;
